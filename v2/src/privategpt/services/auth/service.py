...
from privategpt.shared.security import hash_password, verify_password, create_access_token
...
class AuthService:
    ...
    def login(self, data: UserLogin) -> Token:
        user: User | None = self.db.query(User).filter(User.email == data.email).first()
